...
import StudentCalendar from './src/models/StudentCalendar.js';
import { generateICSString } from './src/utils/icsGenerator.js';
 ...
 * Sync student timetable to DB for live calendar subscription
 */
app.post('/api/calendar/sync', async (req, res) => {
    const { regno, studentName, timetable } = req.body;

    if (!regno || !timetable) {
        return res.status(400).json({ success: false, error: 'Regno and timetable are required' });
    }

    try {
        // Find existing or create new token
        let student = await StudentCalendar.findOne({ regno });
        
        if (!student) {
            // Generate a unique token for this student
            const syncToken = crypto.randomBytes(16).toString('hex');
            student = new StudentCalendar({
                regno,
        ...
        return res.json({
            success: true,
            syncUrl: `${process.env.BACKEND_URL || `http://localhost:${PORT}`}/api/calendar/${student.syncToken}.ics`
        });
    } catch (error) {
 ...
 * Public endpoint to serve the dynamic ICS file for subscription
 */
app.get('/api/calendar/:token.ics', async (req, res) => {
    const { token } = req.params;

    try {
        const student = await StudentCalendar.findOne({ syncToken: token });

        if (!student) {
            return res.status(404).send('Calendar not found');
        }

        const icsString = generateICSString(student.timetable, student.studentName);

        res.setHeader('Content-Type', 'text/calendar; charset=utf-8');
        res.setHeader('Content-Disposition', `attachment; filename="timetable_${student.regno}.ics"`);

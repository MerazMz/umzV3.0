...
const Courses = () => {
    ...
    const [viewMode, setViewMode]     = useState(() => localStorage.getItem('umz_grades_view_mode') || 'grades');
    ...
    useEffect(() => {
        const load = async () => {
            ...
            if (cachedCourses) {
                ...
                if (!cookies) { setLoading(false); setCourses([]); return; }
                try {
                    setLoading(true);
    ...
    const fallbackFiltered = courses.filter(c =>
        !search ||
        c.courseCode?.toLowerCase().includes(search.toLowerCase()) ||
        c.courseName?.toLowerCase().includes(search.toLowerCase())
    );

    const useResultData = allSubjects.length > 0;
            ...
            <main className="flex-1 overflow-y-auto lg:p-0">
                <div className="lg:hidden flex flex-col min-h-full">
                    ...
                    <div className="flex items-center justify-between px-4 py-3 bg-white dark:bg-gray-900 border-b border-gray-100 dark:border-gray-800 sticky top-0 z-30">
                        ...
                        <h1 className="text-base font-bold text-gray-900 dark:text-white tracking-tight">Grades Section</h1>
                        <div className="w-9" />
                    </div>
                            ...
                            {useResultData ? (
                                groupedFiltered.length === 0 ? (
                                    <div className="bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 p-12 flex flex-col items-center gap-2 text-center">
                                        ...
                                        <p className="text-sm font-medium text-gray-400">No records found for this selection</p>
                                        <button 
                                    ...
                                    groupedFiltered.map((group, gIdx) => {
                                        ...
                                        return (
                                            <div key={group.title} className="space-y-4 animate-in fade-in slide-in-from-bottom-2 duration-300">
                                                {/* Stats Card - Only show in grades view when no filters are active */}
                                                            ...
                                                            >
                                                                ...
                                                                <div className="flex-1 min-w-0">
                                                                    <div className="flex items-center gap-2 mb-1">
                                                                        {sub.name && sub.name !== 'N/A' && (
                                                                            <span className="px-1.5 py-0.5 rounded bg-gray-50 dark:bg-gray-700/50 text-[9px] font-bold text-gray-400 dark:text-gray-500 font-mono tracking-tight">
                                                                                {sub.code}
                                                                            </span>
                                                                        )}
                                                                        {sub.attendance && (
                                ...
                                fallbackFiltered.map((course, idx) => (
                                    <div key={idx} className="bg-white dark:bg-gray-800 rounded-2xl border border-gray-100 dark:border-gray-700 p-4 flex items-center gap-4 shadow-sm">
                                        <div className="flex-1 min-w-0">
                                            <p className="text-[10px] font-bold text-gray-400 mb-1">{course.courseCode}</p>
                ...
                <div className="hidden lg:block max-w-5xl mx-auto px-6 lg:px-10 py-8 space-y-6">
                    ...
                    <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden divide-y divide-gray-100 dark:divide-gray-700/60">
                        {fallbackFiltered.map((course, index) => {
